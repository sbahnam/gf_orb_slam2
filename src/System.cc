...
void System::SaveLmkLog(const std::string &filename)
{
    ...
    std::cout << std::endl
              << "Saving " << N << " records to lmk log file " << filename << " ..." << std::endl;
...
void System::SaveTrackingLog(const string &filename)
{
    ...
    cout << endl
         << "Saving tracking log to " << filename << " ..." << endl;
    ...
    for (size_t i = 0; i < mpTracker->mFrameTimeLog.size(); i++)
    {
        ...
        fFrameTimeLog << setprecision(6)
                      ...
                      << mpTracker->mFrameTimeLog[i].lmk_local_map << " "
...
{
    cout << endl
         << "Saving mapping log to " << filename << " ..." << endl;
                              ...
                              size_t &filename_maxlen)
{
    ...
    {
        std::cout << std::endl
                  << "Input directory does not exist: \t" << files_dir << std::endl;
        return;
    }
    ...
    std::cout << std::endl
              << "Found " << filename_list->size() << " files under the given dir with extension " << ext << std::endl;
}
...
void System::LoadMap(const std::string &map_path)
{
    ...
    ifstream fin;
    cout << endl
         << "Loading Reference MapPoints mnIds from RMp_idList.txt" << endl;
    ...
    cout << "reloc kf id = " << firstKFId << endl;
...
void System::SaveMap(const std::string &map_path)
{
    ...
    //        sprintf(str_index, "%08d", count);
    //        count ++;
    //        string RMPfileName = map_path + "/RMapPoints/rmappoint_" + std::string(str_index) + ".yml";
